...
250. [Fixed] Marker detail view can be adapted, need to remove title in marker placement mode, so only emphasis on description. In marker detail view - remove requirements header and leave top box as specifics to marker (question, poll, setup info etc...) Make the box a slightly different color so it stands out - make general, symbol and author info disclosure like symbol details in bottom bar symbol info. With slightly more simple ui design - not soo many boxes etc... but keep relevant info

251. [Fixed] Review all setup tp - sl scale visual indicating swing of current price to sl and tp. Make sure all references and implementations of it in marker list views in left drawer, bottom bar marker section, marker detail view and marker info chart box all have same if not similar design and functionality. Need to double check they all work as should, noticed a couple where current swing was different to chart price levels
