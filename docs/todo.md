...
37. Move the marker detail view away from independent sheet to takeover the already present bottom sheet currently used for symbol control, chart chat, indicators and markers. On marker click the marker detail view will appear fully in the bottom sheet with a x - close icon to resort back to original bottom sheet for chart control. This way we can add a coupl of bottom tabs similar to the current bottom sheet for general marker details, marker chat and possibly marker analysis such as in the future linking other markers, integrating indicator settings into the marker, but this could be added in the future, for now just create the tab space. This way users can minimise the detail view while viewing the selected marker fully. Make it look good and professional and open to expand on later. Will be looking to add a major upgrade for more options etc in marker placement and anlysis so this needs to work well

38. Think the markers need a redesign they dont look fully professional. Want to go down a more generic look for markers a bit like the markers in the ios weather app. We need to either go for a light grey border and dark grey interior or opposite, but we can mix and match based on the marker type, could even user a slight colored gradient background based on specific markers which would allow us to use light grey or dark icon inside that is bold and pronounced in the marker, this way we are not using colors for the icon and will make it look more defined. Also need to ensure the vertical dashed line is not visible inside the marker. We need to remove the user name under the marker as its viewable in the detail view. On clicking the marker it will scale up in a nice animation and could even perform a small wiggle to symbolise selection as well as haptic feedback, the marker could then display more detail inside the marker such as marker name type - so you would have the icon and marker name underneath. Need to ensure other markers in the stack move slightly so they dont get overtaken by enlarged marker