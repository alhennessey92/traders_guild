...
struct TradingChartView: View {
    ...
    /// Whether the cluster expansion sheet is showing
    @State private var showClusterSheet = false
    
    /// The candle index for the cluster being viewed
    @State private var clusterCandleIndex: Int? = nil
    ...
    var body: some View {
        ...
        // Lists all markers at that candle for selection
        .sheet(isPresented: $showClusterSheet) {
            if let candleIndex = clusterCandleIndex {
                ClusterExpansionSheet(
                    markerManager: markerManager,
