...
struct TradingChartView: View {
    ...
    var body: some View {
        ZStack {
            ...
            GeometryReader { geometry in
                ...
                ZStack {
                    ...
                    // FIXED: Show preview during placement AND while sheet is open
                    if isMarkerPlacementMode || pendingMarkerInfo != nil {
                        // Determine which candle index and marker type to use
                        // When sheet is open, use pendingMarkerInfo; otherwise use placement mode values
                        let effectiveCandleIndex = pendingMarkerInfo?.candleIndex ?? previewCandleIndex
                        let effectiveMarkerType = pendingMarkerInfo?.markerType ?? controlViewModel.currentMarkerType
                        ...
                        ZStack {
                            ...
                            if effectiveCandleIndex >= 0 && effectiveCandleIndex < chartData.candles.count {
                                ...
                                if x >= -50 && x <= geometry.size.width + 50 {
                                    ...
                                    // HORIZONTAL LINE PREVIEW - Now handled by MarkerPriceLinesOverlay
                                    // (which sits on top of y-axis with proper price labels)
                                    /*
                                    if let markerType = effectiveMarkerType,
                            ...
                            if effectiveCandleIndex >= 0 && effectiveCandleIndex < chartData.candles.count {
                                ...
                                // FIXED: Pass priceScale for vertical zoom scaling
                    ...
                    // FIXED: Moved before x-axis overlay so it appears behind
                    PriceIndicatorView(
                    ...
                    // FIXED: Moved after price indicator so it appears on top
                    ...
                    // MARKER PRICE LINES OVERLAY
                    // Shows horizontal lines with y-axis price labels for markers
                    // Sits ON TOP of y-axis overlay so labels are visible
                    MarkerPriceLinesOverlay(
                        selectedMarker: markerManager.selectedMarker ?? (tappedMarkerId != nil ? markerManager.markers.first(where: { $0.id == tappedMarkerId }) : nil),
                        previewMarker: {
                    ...
                    // DRAGGABLE TARGET LINE OVERLAY
                    // For prediction markers - shows after entry is selected
                    // User drags vertically to set target/exit price
                    ...
                    if isMarkerPlacementMode && !showMarkerSheet && !isShowingSheet {
                        VStack {
                            HStack(spacing: 12) {
                                ...
                                if isAwaitingTargetSelection {
                                    // CONFIRM TARGET BUTTON - for prediction markers after entry selected
                                    Button(action: {
                                        ...
                                        if let timestamp = coordinateSystem.timestamp(forCandleIndex: previewCandleIndex),
                                           ...
                                           previewCandleIndex < chartData.candles.count,
                                           let markerType = controlViewModel.currentMarkerType,
                                           let targetPrice = predictionTargetPrice {
                                            
                                            let candle = chartData.candles[previewCandleIndex]
                                            markerManager.selectedMarker = nil
                                            
                                            // Create pending marker info with target price
                                            pendingMarkerInfo = PendingMarkerInfo(
                                                candleIndex: previewCandleIndex,
                                                timestamp: timestamp,
                                                price: candle.close,
                                                markerType: markerType,
                                                targetPrice: targetPrice  // NEW: Include target price
                                            )
                                            
                                            // Exit target selection mode and open sheet
                                            isAwaitingTargetSelection = false
                                            impactFeedback.impactOccurred()
                                            isShowingSheet = true
                                    ...
                                    // PLACE BUTTON - standard marker workflow
                                    Button(action: {
                                        ...
                                        if let timestamp = coordinateSystem.timestamp(forCandleIndex: previewCandleIndex),
                                           ...
                                           let markerType = controlViewModel.currentMarkerType {
                                            ...
                                            if let existingMarker = markerManager.existingMarkerOfType(markerType, atCandleIndex: previewCandleIndex) {
                                                // ISSUE #3 FIX: Exit placement mode first, then show custom overlay
                                                // Using custom overlay instead of .alert() to avoid SwiftUI presentation conflicts
                                                // (MainView's bottom sheet is always "presented", blocking alerts)
                                                controlViewModel.cancelMarkerPlacement()
                                                
                                                // Show immediately - no delay needed with custom overlay
                                            ...
                                            // PREDICTION MARKER WORKFLOW:
                                            // Step 1: Select entry candle (current step)
                                            // Step 2: Drag to select target price
                                            // Step 3: Open configuration sheet
                                            if markerType == .predictionTarget {
                                                // Initialize target price above entry for bullish default
                                                let entryPrice = candle.close
                                                let priceRangeSpan = chartData.priceRange.max - chartData.priceRange.min
                                                predictionTargetPrice = entryPrice + (priceRangeSpan * 0.05) // 5% above entry
                                                
                                                // Enter target selection mode
                                                isAwaitingTargetSelection = true
                                                impactFeedback.impactOccurred()
                                            } else {
                                                // STANDARD MARKER WORKFLOW: Open sheet immediately
                                                markerManager.selectedMarker = nil
                                                // FIXED: Capture marker type at placement time to prevent sheet errors
                                                pendingMarkerInfo = PendingMarkerInfo(
                                                    candleIndex: previewCandleIndex,
                                                    timestamp: timestamp,
                                                    price: candle.close,
                ...
                // DUPLICATE MARKER OVERLAY (Custom - No SwiftUI Alert Conflict)
                // .alert() doesn't work because MainView's bottom sheet is always "presented"
                // This custom overlay sits on top of everything without conflicting
        ...
        // FIXED: Now uses captured marker type from pendingMarkerInfo
        .sheet(isPresented: $showMarkerSheet) {
            if let info = pendingMarkerInfo {
                ...
                .onDisappear {
                    ...
                    isShowingSheet = false
                    markerManager.selectedMarker = nil
                    // FIXED: Clear pending marker info to remove preview
        ...
        DragGesture(minimumDistance: 0)
            .onEnded { value in
                ...
                if let marker = ChartMarkerSystem.findMarkerAtLocation(
                    ...
                    // FIXED: Increased from 0.5s to 0.8s to further prevent accidental sheet opens during panning
    ...
    func xAxisOverlay(geometry: GeometryProxy) -> some View {
        ...
        VStack(spacing: 0) {
            ...
            // FIXED: Added edgesIgnoringSafeArea to extend to bottom edge
            Rectangle()
                .fill(Color.black)
...
// MARK: - Marker Price Lines Overlay (On Top of Y-Axis)

/// Shows horizontal price lines for markers with y-axis price labels
/// This view sits ON TOP of the y-axis overlay so labels are visible
/// Matches PriceIndicatorView styling for consistency
struct MarkerPriceLinesOverlay: View {
    ...
    var body: some View {
        GeometryReader { geometry in
            Canvas { context, size in
                // Draw selected marker's horizontal line
                if let marker = selectedMarker,
                   marker.type.hasHorizontalLine,
                   let candle = getCandleForMarker(marker) {
                    
                    if let linePrice = marker.getLinePrice(candle: candle) {
                        drawPriceLine(
                            context: context,
                            size: size,
                            price: linePrice,
                ...
                // Draw preview marker's horizontal line (when placing)
                if let preview = previewMarker {
                    let linePrice: Double
                    switch preview.type.lineSource {
                    case .candleOpen:
                        linePrice = preview.candle.open
                    case .candleClose:
                        linePrice = preview.candle.close
                    case .candleHigh:
                        linePrice = preview.candle.high
                    case .candleLow:
                        linePrice = preview.candle.low
                    case .custom, .none:
                        linePrice = preview.candle.close
                    }
                    
                    // Add "Entry" label for prediction markers
    ...
    private func drawPriceLine(
        ...
        color: Color,
        isDashed: Bool,
        label: String? = nil  // NEW: Optional label like "Entry" or "Target"
    ) {
        ...
        let lineEndX = size.width - 60  // Stop before y-axis (matches PriceIndicatorView)
        ...
        // Draw y-axis price label with optional text label
        let labelX = size.width - 35
        
        // Format price using symbol-aware formatting
        let priceText = chartData.formatPrice(price)
        
        // Combine label + price if label provided (e.g. "Entry 1.2345" or "Target 1.2500")
        ...
        // Draw text (white text on colored background)
        context.draw(
            Text(displayText)
                .font(.system(size: 10, weight: .semibold, design: .monospaced))
                .foregroundColor(.white),
...
/// Draggable horizontal line for selecting prediction target/exit price
/// Shows after entry candle is selected for prediction markers
/// User drags vertically to set target price before opening config sheet
    ...
    var body: some View {
        GeometryReader { geometry in
            Canvas { context, size in
                ...
                let lineEndX = size.width - 60  // Stop before y-axis
                
                // Draw horizontal target line (orange/red for target)
                let linePath = Path { path in
                    path.move(to: CGPoint(x: 0, y: y))
                    path.addLine(to: CGPoint(x: lineEndX, y: y))
                }
                
                // Solid line, thicker when dragging
                ...
                // Draw y-axis price label with "Target" text
                let labelX = size.width - 35
                ...
                // Draw label background (orange for target)
                let roundedPath = Path(roundedRect: labelRect, cornerRadius: 4)
                ...
                // Draw drag handle indicator on left side
            ...
            .gesture(
                DragGesture(minimumDistance: 0)
                    .onChanged { value in
                        ...
                        // Convert touch y to price
                        let touchY = value.location.y
                        let price = coordinateSystem.price(atYPosition: touchY)
                        // Clamp to chart bounds
                        let clampedPrice = max(chartData.priceRange.min, min(chartData.priceRange.max, price))
...
//        context.stroke(linePath, with: .color(color.opacity(0.6)), style: strokeStyle)
//        
//        // Draw y-axis price label (matching PriceIndicatorView exactly)
//        let labelX = size.width - 35
//        let labelRect = CGRect(
//            x: labelX - 35,
//            y: y - 11,
