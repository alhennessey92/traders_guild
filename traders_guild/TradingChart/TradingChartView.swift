...
//                                            .font(.headline)
    ...
    /// Labels now align perfectly with grid lines
    ...
    private func yAxisOverlay(geometry: GeometryProxy) -> some View {
        HStack {
            ...
            Canvas { context, size in
                ...
                // Use SAME price step calculation as grid for perfect alignment
                let priceStep = calculateNicePriceStep(range: priceRange.max - priceRange.min)
                
                // Start from a rounded price value (same as grid)
                ...
                if priceStep >= 1.0 {
                    ...
                    decimalPlaces = 3
                } else if priceStep >= 0.0001 {
                    decimalPlaces = 4
                } else {
                    decimalPlaces = 5
                }
                
                // Draw labels at nice price intervals (matching grid lines exactly)
                var currentPrice = startPrice
                while currentPrice <= priceRange.max + priceStep {
                    // Calculate Y position using SAME formula as grid
                    ...
                    if y >= -20 && y <= size.height + 20 {
                        let priceText = String(format: "%.\(decimalPlaces)f", currentPrice)
                        context.draw(
                            Text(priceText)
                                .font(.system(size: 10))
    ...
    /// Shows time labels at zoom-adaptive intervals
    /// - Parameter geometry: Chart view geometry
    ...
    private func xAxisOverlay(geometry: GeometryProxy) -> some View {
        VStack {
            ...
            Canvas { context, size in
                ...
                // Use SAME candle stride as vertical grid lines for perfect alignment
    ...
    private func drawGrid(context: GraphicsContext, size: CGSize) {
        ...
        let gridPath = Path { path in
            // Vertical grid lines (aligned with time) - NOW ZOOM-AWARE
            // Calculate candle stride based on horizontal zoom level
            let candleStride = calculateCandleStride()
            let verticalSpacing = totalCandleWidth * CGFloat(candleStride)
            ...
            // Horizontal grid lines (aligned with ACTUAL PRICES)
            // Calculate nice price step for grid lines
            let priceStep = calculateNicePriceStep(range: priceRange.max - priceRange.min)
            
            // Start from a rounded price value
            let startPrice = floor(priceRange.min / priceStep) * priceStep
            
            // Draw grid lines at nice price intervals
            var currentPrice = startPrice
            while currentPrice <= priceRange.max + priceStep {
                // Calculate Y position using SAME formula as candles
                let normalizedPrice = (currentPrice - priceRange.min) / (priceRange.max - priceRange.min)
                let y = size.height - (CGFloat(normalizedPrice) * scaledHeight) - totalVerticalOffset
    ...
    private func calculateNicePriceStep(range: Double) -> Double {
        ...
        // When zoomed in, show more lines. When zoomed out, show fewer.
        let baseTargetLines: Double = 10.0
        let zoomFactor = gestureState.priceScale
        
        // Scale inversely with zoom: more zoom = more lines
        let targetLines: Double
        if zoomFactor > 2.0 {
            targetLines = baseTargetLines * 1.5  // 15 lines when highly zoomed in
        } else if zoomFactor > 1.5 {
            targetLines = baseTargetLines * 1.3  // 13 lines
        } else if zoomFactor < 0.7 {
            targetLines = baseTargetLines * 0.7  // 7 lines when zoomed out
        } else {
            targetLines = baseTargetLines  // 10 lines at normal zoom
        }
