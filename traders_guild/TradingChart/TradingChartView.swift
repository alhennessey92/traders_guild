...
struct TradingChartView: View {
    ...
    var body: some View {
        ...
        .onAppear {
            ...
            // Generate sample guild member markers for testing
            // This shows markers from other guild members on the chart
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
                if markerManager.markers.isEmpty && !chartData.candles.isEmpty {
                    markerManager.generateSampleGuildMarkers(candles: chartData.candles, count: 5)
                }
            }
