...
struct TradingSymbolDTO: Identifiable, Codable, Equatable {
    ...
    // Visual identity
    let iconName: String?               // SVG asset name (e.g., "icon_btc", "icon_eurusd")
    let primaryColor: String            // Hex color for primary branding (e.g., "#F7931A")
    let secondaryColor: String          // Hex color for secondary/accent (e.g., "#000000")
...
extension TradingSymbolDTO {
    ...
    static func fromLegacy(_ symbol: TradingSymbol, currentPrice: Double = 0) -> TradingSymbolDTO {
        // Determine icon name based on asset class and ticker
        let iconName: String?
        switch symbol.assetClass {
        case .forex:
            iconName = "icon_\(symbol.symbol.lowercased())" // e.g., "icon_eurusd"
        case .crypto:
            iconName = "icon_\(symbol.symbol.lowercased())" // e.g., "icon_btcusd"
        default:
            iconName = nil // No icons for stocks, commodities, indices
        }
        
        // Default colors (will be overridden in sample data)
        let (primaryColor, secondaryColor) = defaultColors(for: symbol.symbol, assetClass: symbol.assetClass)
    ...
    /// Helper to get default colors based on symbol
    private static func defaultColors(for ticker: String, assetClass: TradingSymbol.AssetClass) -> (String, String) {
        // Basic defaults by asset class
        switch assetClass {
        case .forex:
            return ("#2E5CFF", "#FFFFFF") // Blue and white
        case .crypto:
            return ("#F7931A", "#000000") // Orange and black (Bitcoin colors)
        case .stocks:
            return ("#5856D6", "#FFFFFF") // Purple and white
        case .commodities:
            return ("#FFD60A", "#000000") // Gold and black
        case .indices:
            return ("#007AFF", "#FFFFFF") // Blue and white
        case .futures:
            return ("#34C759", "#FFFFFF") // Green and white
        }
    }
}
