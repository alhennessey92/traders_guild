...
class LeftDrawerViewModel: ObservableObject {
    ...
    func preloadData(for guildId: UUID, appState: AppState) async {
        ...
        do {
            let (fetchedMembers, fetchedWatchlist, fetchedTradingWatchlist, fetchedPersonalTradingWatchlist, fetchedUserNotifications, fetchedStatistics) = try await (
                membersTask,
                watchlistTask,
...
//            group.addTask {
//                do {
//                    let fetched = try await appState.fetchGuildEvents(guildId: guildId)
//                    await MainActor.run { self.upcomingEvents = fetched }
//                } catch is CancellationError {
//                    // Silent - expected during navigation
//                } catch {
...
//    
//    /// Refresh only events - use this in EventsView
//    func refreshEvents(guildId: UUID, appState: AppState) async {
//        do {
//            let fetched = try await appState.fetchGuildEvents(guildId: guildId)
//            await MainActor.run {
//                self.upcomingEvents = fetched
